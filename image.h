...
public:
    ...
    Pixel& GetPixel(int x, int y) const {
        ...
        assert(ValidCoord(real_x, real_y));
        return data.pixels[real_y * width + real_x];
    }
    Pixel GetPixel_rotate(int x, int y) const {
        int real_x = x;
        int real_y = y;
    ...
    void SetPixel (int x, int y, Pixel p) const { 
        ...
        assert(ValidCoord(x,y));  data.pixels[y*width + x] = p; }
    ...
    Pixel Sample_for_rotate(double u, double v);
};
